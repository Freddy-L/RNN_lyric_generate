...
max_words = 3000
epochs = 50
poetry_file = 'reversed_text.txt'
save_dir = 'reversed_text_log'
...
class Data:
    ...
    def load(self):
        ...
        words, _ = zip(*count_pairs)    # 按字频由高到低排列，并取出字列表，words为list[str]
        ...
        self.poetrys_vector = [list(map(self.char2id, poetry)) for poetry in self.poetrys] # 由各个诗句分字后各个字的id构成
        ...
        for i in range(self.n_size):
            ...
            for row in range(self.batch_size):
                if len(batches[row]) < length:
                    ...
                    batches[row][len(batches[row]): length] = [self.unknow_char] * r    # batch里长度不够的句子尾部填充unk
            xdata = np.array(batches)
            ydata = np.copy(xdata)
            ydata[:, :-1] = xdata[:, 1:] # ydata在藏头诗情况下，相当于对句子进行了左移，只保留了第二个字符到末尾字符，并且末位字符重复
            self.x_batches.append(xdata)    # [n_batches, batch_size, seq_len]
            self.y_batches.append(ydata)


class Model:
    def __init__(self, data, model='lstm', infer=False):
        self.rnn_size = 128     # hidden的维度
        self.n_layers = 2
        ...
        # outputs性质[batch_size, max_time, hidden_size]
        # final_states是最后一个rnn_cell的hidden值

        self.output = tf.reshape(outputs, [-1, self.rnn_size])  # [batch_size * max_time, hidden_size] 每条数据的各个位置的hidden
        self.logits = tf.matmul(self.output, softmax_w) + softmax_b  # [batch_size * max_time, words_size]
        self.probs = tf.nn.softmax(self.logits)     # [batch_size * max_time, words_size]
        self.final_state = final_state      # [batch_size, hidden]
        pred = tf.reshape(self.y_tf, [-1])   # target, [batch_size * max_time]
        # seq2seq
        loss = seq2seq.sequence_loss_by_example([self.logits],
                                                [pred],
                                                [tf.ones_like(pred, dtype=tf.float32)],)    # [batch_size * max_time, 1]

        self.cost = tf.reduce_mean(loss)    # 求batch内各个位置平均损失
        ...
        for epoch in range(epochs):
            ...
            for batche in range(data.n_size):
                ...
                if (epoch * data.n_size + batche) % 1000 == 0 \
                        or (epoch == epochs-1 and batche == data.n_size-1):     # 每一千个batch以及最后一个batch记录模型
